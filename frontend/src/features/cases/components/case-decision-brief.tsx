...
import { responseDraftPresentation } from "@/features/cases/case-presentation";
import {
  AlertCircle,
  CheckCircle2,
  ChevronDown,
...
export function CaseDecisionBrief({ workspace }: { workspace: CaseWorkspace }) {
  ...
  return (
    <section
      ...
      className="px-4 py-6 sm:px-6 lg:px-7 lg:py-8"
    >
      ...
      {!workspace.proposal ? (
        <section
          aria-labelledby="analysis-not-prepared-heading"
          className="py-10"
        >
          <Sparkles aria-hidden="true" size={22} className="text-info" />
          <h2
            id="analysis-not-prepared-heading"
            className="mt-4 text-base font-semibold text-primary"
          >
            Analysis not prepared
          </h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-secondary">
            Analyze the case to verify the current records, check the relevant
            policy, and prepare the next step.
          </p>
        </section>
      ) : (
        <>
              ...
              <div className="mt-4 divide-y divide-border border-y border-border">
                {workspace.evidence.map((item) => (
                  ...
                  >
                    ...
                    <StatusLabel
                      tone={
                        item.freshness === "current" ? "success" : "warning"
                      }
                    >
                      {item.freshness}
                    </StatusLabel>
